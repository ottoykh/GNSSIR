...
def rinex_convert(input_folder: str, gfzrnx_exe: str, doy_start: int, doy_end: int, station_name: str, output_folder: str = None) -> None:
    """
    ...
    Args:
        input_folder (str): The path to the folder containing the input RINEX files.
            Filenames are expected to follow the pattern: {station_name}00HKG_R_2025{doy_str}0000_01D_30S_MO.rnx
    ...
    for doy in range(doy_start, doy_end + 1):
        ...
        input_file = os.path.join(input_folder, f"{station_name}00HKG_R_2025{doy_str}0000_01D_30S_MO.rnx")

        # Check if the input file exists
        if os.path.exists(input_file):
            output_file = os.path.join(output_folder, f"{station_name.lower()}{doy_str}0.25o")
